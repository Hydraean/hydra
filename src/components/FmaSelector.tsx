...
const FMASelector = (props: any) => {
  ...
  const selectFMA = (fma: any) => {
    ...
    let fmaIncidents = props.data.incidents_overview.find(
      (x: any) => x.fma === fma.fma
    ).records;
    props.setShapeChartData([]);
    setTimeout(() => {
      props.setShapeChartData(
        fmaIncidents.length > 0
          ? [...fmaIncidents, ...noDataEntry]
          : [...noDataEntry, ...noDataEntry]
      );
    }, 200);
      ...
      <div className="modal-drawer slide-in-right">
        ...
        <div className="selector-content">
          ...
          {fma_list.map((x: any, index: number) => {
            return (
              <div
                className={`selector-item fade-in ${
                  props.currentFMA.fma === x.fma ? "active" : ""
                }`}
                key={index}
                style={{
